...
func TestInject_MethodPolicyAllowsListedMethod(t *testing.T) {
	...
	f.setServices(t, "v1", []broker.Service{{
		...
		Methods: []string{"GET"},
		Auth:    broker.Auth{Type: "bearer", Token: "GOOGLE_TOKEN"},
	}})
	f.setCred(t, key32, "v1", "GOOGLE_TOKEN", "ya29")

	p := NewStoreCredentialProvider(f, key32)
	res, err := p.Inject(context.Background(), "v1", "www.googleapis.com", 443, "GET", "/calendar/v3/calendars/primary/events")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if res.MatchedName != "calendar-read" {
		t.Fatalf("MatchedName = %q, want calendar-read", res.MatchedName)
	}
}

func TestInject_MethodPolicyDeniesUnlistedMethod(t *testing.T) {
	key32 := make32(0x92)
	f := newFakeCredStore()
	f.setServices(t, "v1", []broker.Service{{
		Name:    "calendar-read",
	...
	p := NewStoreCredentialProvider(f, key32)
	_, err := p.Inject(context.Background(), "v1", "www.googleapis.com", 443, "POST", "/calendar/v3/calendars/primary/events")
	if !errors.Is(err, ErrServiceMethodDenied) {
		t.Fatalf("err = %v, want ErrServiceMethodDenied", err)
	}
...
func TestInject_MethodPolicyDoesNotFallThroughToBroaderRule(t *testing.T) {
	key32 := make32(0x93)
	f := newFakeCredStore()
	f.setServices(t, "v1", []broker.Service{
		{
		...
		},
		{
			...
			Host: "gmail.googleapis.com",
			Path: "/*",
			Auth: broker.Auth{Type: "bearer", Token: "BROAD_TOKEN"},
		},
	})
	f.setCred(t, key32, "v1", "GOOGLE_TOKEN", "narrow")
	f.setCred(t, key32, "v1", "BROAD_TOKEN", "broad")

	p := NewStoreCredentialProvider(f, key32)
	_, err := p.Inject(context.Background(), "v1", "gmail.googleapis.com", 443, "POST", "/gmail/v1/users/me/messages")
	if !errors.Is(err, ErrServiceMethodDenied) {
		t.Fatalf("err = %v, want ErrServiceMethodDenied", err)
	}
	if f.getCredentialCalls != 0 {
...
func TestInject_PathBasedDisambiguation(t *testing.T) {
	...
	for _, tc := range cases {
		res, err := p.Inject(context.Background(), "v1", "slack.com", 0, "GET", tc.path)
		if err != nil {
