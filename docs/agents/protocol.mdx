...
```json Response
{
  "vault": "my-vault",
  "services": [
    { "name": "stripe", "host": "api.stripe.com", "methods": ["*"] },
    { "name": "github", "host": "*.github.com", "methods": ["*"] },
    { "name": "slack-bot", "host": "slack.com/api/*", "methods": ["POST"] },
    { "name": "slack-conn", "host": "slack.com/api/apps.connections.*", "methods": ["GET"] }
  ],
  "available_credentials": ["GITHUB_TOKEN", "SLACK_BOT_TOKEN", "SLACK_CONNECTION_TOKEN", "STRIPE_KEY"]
}
```

- **`services`** lists the services the agent can reach through Agent Vault (each entry has `name`, `host`, and `methods`; `host` carries the joined inline form, so a path-scoped service shows up as e.g. `slack.com/api/*`). `methods: ["*"]` means any method. Requests to any other host go direct. When two services share the same bare host but scope to different paths (e.g. Slack here), distinguish them by `name` in subsequent operations.
...
| 403 `method_not_allowed` | Host/path matched a service, but the request method is outside `services[].methods` | Retry with an allowed method or propose an allowlist change. |
