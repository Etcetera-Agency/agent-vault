...
- **Methods**: Optional HTTP method allowlist. Omit `methods` or use `methods: ["*"]` for any method. Lowercase input is normalized to uppercase; empty, duplicate, invalid, or mixed wildcard entries are rejected.
...
## Optional quota and account config

Services may carry quota and account-pool policy. These fields are optional;
services that omit them behave as unlimited.
...
All quota numbers are non-negative. `daily_cap`, `monthly_cap`, `rpm`, and
`concurrency` can be set independently; if both daily and monthly caps are set,
`monthly_cap` must be greater than or equal to `daily_cap`. `rotation` accepts
`least_used` or `round_robin`.

Account pools reference credential keys only. `credential_key` must name an
existing vault credential, account `id` values must be unique within the service,
and Agent Vault never stores or returns credential values in the quota/account
config.

When a service has a `quota`, Agent Vault enforces only the fields you set. A
reached `daily_cap` or `monthly_cap`, an exhausted `rpm` window, or a full
`concurrency` slot returns `429` with `X-Vault-Quota-Exhausted: <service>` and
`Retry-After`. Counts are reconstructed from request logs on restart. Set a
provider-side ceiling first when the upstream supports it, then set the Agent
Vault quota as the local guardrail agents cannot bypass.

When `accounts` are configured, Agent Vault selects a non-exhausted account
before injecting credentials. `least_used` chooses the account with the lowest
current window usage; `round_robin` walks the account list and skips exhausted
or cooling accounts. If a replay-safe request (`GET` or `HEAD`) receives an
upstream `429`, Agent Vault cools down that account using `Retry-After` and
retries with another available account. When every account is exhausted or
cooling, Agent Vault emits one SMTP operator alert per service per day when SMTP
notifications are configured.

Vault admins can manage the same quota fields in the **Services** tab. The
editor accepts blank quota fields as unset, uses credential key selectors for
account pools, and never displays credential values. The service table shows
daily/monthly usage against configured caps plus each account's available,
exhausted, or cooling state.
...
Service list and discovery responses render unrestricted methods as `methods: ["*"]` so operators and agents can audit the policy without inferring from an omitted field. At proxy time, the most-specific host/path/port match is evaluated first; if that service excludes the request method, Agent Vault returns `403 method_not_allowed` and does not fall through to a broader rule or unmatched-host passthrough.
...
Vault admins can add, edit, disable, and delete services from the vault's Services tab. The editor saves the full allowlist rule: service name, host/path pattern, methods, enabled state, auth type, credential key references, and substitutions. Credential selectors and inputs show credential key names only; decrypted credential values are never displayed.
