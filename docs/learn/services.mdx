...
- **Methods**: Optional HTTP method allowlist. Omit `methods` or use `methods: ["*"]` for any method. Lowercase input is normalized to uppercase; empty, duplicate, invalid, or mixed wildcard entries are rejected. This field is stored and returned by service APIs, but method enforcement lands in the next slice.
