...
9. ~~Fix `agent-vault mail-proxy` controlled shutdown so active IMAP/SMTP relay connections are tracked and closed after `--shutdown-timeout`, not only listeners.~~ **Resolved in `fix-mail-proxy-review-gaps` slice 1.**
10. ~~Make SMTP upstream STARTTLS verification derive `ServerName` from the configured `--smtp-upstream` host.~~ **Resolved in `fix-mail-proxy-review-gaps` slice 2.**
