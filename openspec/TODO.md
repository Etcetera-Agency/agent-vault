...
9. Implement multi-account quota rotation from `add-credential-rotation`: least-used/round-robin account selection, cooldown, bounded upstream-429 failover, and exhaustion alerts.
10. Implement quota management UI from `add-quota-ui`: quota/account editors plus usage/exhaustion read path without credential values.
## Resolved
