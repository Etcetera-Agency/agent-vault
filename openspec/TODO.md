...
9. Implement runtime quota enforcement from `enforce-egress-quota`: request-log-seeded daily/monthly counters, rpm, concurrency, and HTTP 429 exhaustion responses.
10. Implement multi-account quota rotation from `add-credential-rotation`: least-used/round-robin account selection, cooldown, bounded upstream-429 failover, and exhaustion alerts.
11. Implement quota management UI from `add-quota-ui`: quota/account editors plus usage/exhaustion read path without credential values.
## Resolved

12. ~~Fix `agent-vault mail-proxy` controlled shutdown so active IMAP/SMTP relay connections are tracked and closed after `--shutdown-timeout`, not only listeners.~~ **Resolved in `fix-mail-proxy-review-gaps` slice 1.**
13. ~~Make SMTP upstream STARTTLS verification derive `ServerName` from the configured `--smtp-upstream` host.~~ **Resolved in `fix-mail-proxy-review-gaps` slice 2.**
14. ~~Decide how `/discover` represents unrestricted methods.~~ **Resolved: canonical `["*"]`.** Storage keeps unrestricted as empty/omitted (backward compatible); `["*"]` is accepted on input as the sole-element alias (normalized to empty) and rendered on every surface — `/discover`, service list, CLI, UI, docs. Captured in `add-service-method-field` and `surface-service-methods`.
15. ~~Decide UI copy for unrestricted method policy.~~ **Resolved: show the `["*"]` token** as the badge/value, with an `Any method` tooltip/label for readability.
